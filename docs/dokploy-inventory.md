...
- `npm run inventory:reconcile` immediately retries outstanding holds, including recently checked ones, and reports checked, expired, paid, still-open and failed counts. It exits unsuccessfully when any hold fails. If you find the matching Checkout Session in Stripe, `npm run inventory:reconcile -- cs_SESSION_ID` also expires that checkout when it is unpaid, open and past the hold timeout, then reconciles it using its metadata and line items. Do not manually release uncertain holds without checking Stripe's records.
- Startup logs print the deployed hold timeout. Failed Stripe expiration requests log only diagnostic type/code/status/request identifiers, so an API error can be investigated without exposing credentials or customer data.
- Application database connections use the name `bella-vita-inventory`, a five-second lock timeout and a fifteen-second statement timeout. A locked reservation is reported as failed and retried later; it does not stop cleanup from checking the other reservations. These limits apply after redeployment and do not terminate existing blocking database sessions.
